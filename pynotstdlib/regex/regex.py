...
def match(string: str, using: str):
    ...
    return regex_answers.compute_if_not_exists(key = string + using, fun = lambda: re.match(my_regex, string))
...
def findall(string: str, using: str):
    ...
    # Stores 20 compiled strings, and 30 'matches'.
    my_regex = regex_strings.compute_if_not_exists(key = using, fun = re.compile, pattern = using)
    return regex_answers.compute_if_not_exists(key = string + using, fun = lambda: re.findall(my_regex, string))
...
def search(string: str, using: str):
    ...
    # Stores 20 compiled strings, and 30 'matches'.
    my_regex = regex_strings.compute_if_not_exists(key = using, fun = re.compile, pattern = using)
    return regex_answers.compute_if_not_exists(key = string + using, fun = lambda: re.search(my_regex, string))
